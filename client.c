...
void handleServer() {
	/* 
		when we got a signal from the server we call this function, and handle the server -
		read data from server, and print it to the screen.
    */
	int pid, status;
	char client_file_name[SIZE_NAME], client_file_data[SIZE_FILE], client_pid[5];
	...
	if (fid_client_file == -1) { write(2, "Cannot open to_client.txt\n", 26); exit(-1); }
	...
	pid = fork();
	if (pid == 0) {
		execl("/bin/rm", "/bin/rm", client_file_name);
	}
	else {
...
int main(int argc, char* argv[]) {
	...
	if (ret_val_kill == -1) { 
		...
		if (pid_rm == 0) { // child to delete to_srv.txt if we did not send it
			execl("/bin/rm", "/bin/rm", "to_srv.txt");
		}
		else {
			waitpid(pid_rm, &status, NULL);
		}
		close(fid_srv_file);
		exit(-1); 
	}
